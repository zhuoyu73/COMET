...
function [imagePhantom,sens_rast] = RasterizePhantom(phantom,res,s,MATLAB_SL_CONSISTENT)
%Input check
...
for indRegion = 1:Nreg
    ...
    switch region.type
        case {'ellipse'}
            ...
            imagePhantom(support_ell) = imagePhantom(support_ell)+region.weight;
        case{'polygon'}
            imagePhantom = imagePhantom + region.weight*double(inpoly(X,Y,region.vertex(:,1),region.vertex(:,2)));
        case {'bezier'}
            imagePhantom = imagePhantom + region.weight*double(InBezierDefinedRegion(region,X,Y));
        otherwise
...
imagePhantom = imagePhantom.*sens_rast;

% Output Check
if nargout==0
    imagesc(abs(imagePhantom));colormap gray;colorbar;axis image;drawnow;
