classdef recoInfoIsmrmrd
    ...
    methods
        function obj = recoInfoIsmrmrd(filename, varargin)
            ...
            if(isfield(hdr.encoding.encodingLimits,'kspace_encoding_step1'))
                obj.nShots = hdr.encoding.encodingLImits.kspace_encdoing_step1 + 1;
            else
                obj.nShots = 1;
            end
                        
            if(isfield(hdr.encoding.encodingLimits,'kspace_encoding_step2'))
            	obj.nPartitions = hdr.encoding.encodingLimits.kspace_encoding_step2.maximum + 1;
            else
                obj.nPartitions = 1;
            end
