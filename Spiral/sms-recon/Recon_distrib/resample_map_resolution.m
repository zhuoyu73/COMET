function mapout = resample_map_resolution(mapin,Nout,Noutsl); 
% function mapout = resample_map_resolution(mapin,recoInfo.N,recoInfo.nsl); 
% This function resamples a map to match a target resolution
% WARNING: IT ASSUMES THAT THE FOV WAS THE SAME as the target resolution and that the 
%   number of slices and slice thickness is also the same.
...
input_vec = ([0:nx-1] - nx/2 + 1/2)./nx; 
output_vec = ([0:Nout-1] - Nout/2 + 1/2)./Nout; 
...
if flag_phase
   mapout = mapout_mag.*exp(i*mapout_phs);
else
   mapout = mapout_mag;
end
...
if ~(nsl == Noutsl)
  ...
  if flag_phase
    mapout2 = mapout2_mag.*exp(i*mapout2_phs);
  else
    mapout2 = mapout2_mag;
  end
