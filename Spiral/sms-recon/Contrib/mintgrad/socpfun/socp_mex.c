...

{
  ...
  if (firstcall) {
    ...
    fprintf(stdout, "COPYRIGHT (c) 1997, Miguel Lobo, Lieven Vandenberge, "
                    "Stephen Boyd.\n\n");
    firstcall = 0;
  }
  ...
  N = mxCalloc(L, sizeof(ptrdiff_t)); /* new N of type int */
  for (i = 0, j = 0; i < L; ++i) {
  ...
  plhs[4] =
      mxCreateDoubleMatrix(1, 3, mxREAL); /* time stats: utime, stime, iters */
  time = mxGetPr(plhs[4]);
...
/* initialize user time and system time */
  ...
  time[0] =
      -(double)stats.ru_utime.tv_sec - ((double)stats.ru_utime.tv_usec) / 1e6;
  time[1] =
      -(double)stats.ru_stime.tv_sec - ((double)stats.ru_stime.tv_usec) / 1e6;
#else
  ...
  info_socp = socp(L, N, n, f, A, b, x_opt, z_opt, abs_tol, rel_tol, target,
                   &iter, Nu, &info_exit, out_mode, hist, dblwork, intwork);
