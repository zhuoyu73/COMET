function [img, resid, roughness] = phaseCorrectedReconCIVIC(rInfo, sen, mask, FM, PMaps, varargin)
            ...
            for nn = 1:length(echoesToRecon)
               for oo = 1:length(repetitionsToRecon)
                  ...
                  if (rInfo.nPartitions > 1) || (rInfo.multibandFactor > 1) % CLJ: I think this is the correct config for MB imaging
                     G = NUFFT(reshape(rInfo.kx(rInfo.dataMask,:,:,slc,avg,phs,eco,rep),rInfo.shotLength,[]), ...
                        reshape(rInfo.ky(rInfo.dataMask,:,:,slc,avg,phs,eco,rep),rInfo.shotLength,[]), ...
                        reshape(rInfo.kz(rInfo.dataMask,:,:,slc,avg,phs,eco,rep),rInfo.shotLength,[]), ...
                        rInfo.N, ...
                        rInfo.N, ...
                        Nz, ...
                  ...
                  %kx = reshape(rInfo.kx(rInfo.dataMask,:,:,slc,avg,phs,eco,rep),sum(rInfo.dataMask(:)),rInfo.nPartitions*rInfo.nShots);
                  %ky = reshape(rInfo.ky(rInfo.dataMask,:,:,slc,avg,phs,eco,rep),rInfo.nPartitions*rInfo.nShots);
                  %kz = reshape(rInfo.kz(rInfo.dataMask,:,:,slc,avg,phs,eco,rep),rInfo.nPartitions*rInfo.nShots);
                  ...
                  % Note that we have to resample the images that we need as
                  % we go, otherwise we will run out of memory!
                  P = zeros(rInfo.N,rInfo.N,rInfo.nPartitions,rInfo.nShots,rInfo.nPartitions);

                  for jj = 1:rInfo.nPartitions
                      for ii = 1:rInfo.nShots
                          P(:,:,:,ii,jj) = angle(resampleMapNavUp(PMaps(:,:,:,ii,jj,slc,avg,phs,eco,rep),rInfo,rInfo));
                      end
                  end
                  
                  P = reshape(P,rInfo.N,rInfo.N,Nz,rInfo.nShots*rInfo.nPartitions);
                  ...
                  ww = weight_vor(col(rInfo.kx(rInfo.dataMask,1,1,slc,avg,phs,eco,rep)),col(rInfo.ky(rInfo.dataMask,1,1,slc,avg,phs,eco,rep)),2);
                  ww = repmat(ww(:),1,rInfo.nCoils,rInfo.nShots,rInfo.nPartitions);
