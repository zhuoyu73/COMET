function [r2final, wefinal] = jointestimation_r2_we_multi_echo_fmincon_S0(data,maskTE,rInfo, FM, sen, r2map,han_flag, numloop,numloop_ext,mask)
...
for ii=1:1
    ...
    A_t0s  = sense_svd(A_t0,sen,1);
    data0s = prepData(A_t0s,col(data_t0));
    
    if ii ==1
        im_rec = qpwls_pcg(xinit(:),A_t0s,1,col(data0s),0,C,1,40,[],0);
    else
    ...
    fobj = @(x) (R2StarFMCostFunctionS0(x,data,sen,rInfo,Rr2,Rwe,RS0,maskTE,mask,nc_svd,tt_ext));
    options = optimoptions('fmincon','Algorithm','interior-point','Display','iter-detailed','SpecifyObjectiveGradient',true,'HessianApproximation','lbfgs','MaxIter',100,'MaxFunEvals',300,'TolFun',1E-3,'TolX',1E-3);
    xinitFmincon = vertcat(col(r2temp(:,1,ii)),col(wetemp(:,1,ii)),real(col(B)),imag(col(B)));
    lBounds = vertcat(col(zeros(size(r2temp(:,1,ii)))),col(-10000*ones(size(wetemp(:,1,ii)))),-1*col(100*ones(size(r2temp(:,1,ii)))),-1*col(100*ones(size(r2temp(:,1,ii)))));
    uBounds = vertcat(col(500*ones(size(r2temp(:,1,ii)))),col(10000*ones(size(wetemp(:,1,ii)))),col(100*ones(size(wetemp(:,1,ii)))),col(100*ones(size(wetemp(:,1,ii)))));
    
    [x] = fmincon(fobj,xinitFmincon,[],[],[],[],lBounds,uBounds,[],options);
    xlen = length(x);
    r2temp(:,1,ii+1)=x(1:xlen/4);
    wetemp(:,1,ii+1)=x(xlen/4+1:(2*xlen/4));
    s0real=x((2*xlen/4+1:3*xlen/4));
    s0imag=x((3*xlen/4+1:end));
    s0temp(:,1,ii+1)=s0real + 1j*s0imag;
    
    r2result(:,1,ii+1)=x(1:xlen/4);
    weresult(:,1,ii+1)=x(xlen/4+1:(2*xlen/4));
    s0result(:,1,ii+1)=s0temp(:,1,ii+1);
