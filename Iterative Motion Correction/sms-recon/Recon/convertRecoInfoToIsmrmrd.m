function convertRecoInfoToIsmrmrd(filename,rInfo,sen, FM, PhaseMaps,v_svd, varargin)
...
partitionsToRecon = p.Results.partitionsToRecon;
...
if nargin > 4 && ~isempty(PhaseMaps)   %GM
    PMaps = ismrmrd.NDArray(col(PhaseMaps));
    dset.appendArray('PhaseMaps', PMaps);
end

% if exists append v to file %GM
if nargin > 5 && ~isempty(v_svd)
    v = ismrmrd.NDArray(col(v_svd));
    dset.appendArray('v', v);
end 
            ...
            for slcIdx = 1:length(slicesToRecon)
                for parIdx = 1:length(partitionsToRecon)
                    for shotIdx = 1:length(shotsToRecon)
                        ...
                        if slcIdx == length(slicesToRecon) && parIdx == length(partitionsToRecon) && shotIdx == length(shotsToRecon)
                        ...
                        if slcIdx == length(slicesToRecon) && par == length(partitionsToRecon) && shot == length(shotsToRecon) && ...
                                rep == length(repetitionsToRecon) && avg == length(averagesToRecon) ...
