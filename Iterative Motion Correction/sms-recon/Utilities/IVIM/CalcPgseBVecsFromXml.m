function [bvecs] = CalcPgseBVecsFromXml(filename)
...
for ii = 1:numDiffusionEncodings
    ...
    diffDirectionRead(ii)  = str2num(DiffusionInfo.diffusionScheme.diffEncodingTable.diffEncode{1,ii}.Attributes.Read);
    diffDirectionPhase(ii) = str2num(DiffusionInfo.diffusionScheme.diffEncodingTable.diffEncode{1,ii}.Attributes.Phase);
    diffDirectionSlice(ii) = str2num(DiffusionInfo.diffusionScheme.diffEncodingTable.diffEncode{1,ii}.Attributes.Slice);
end

% Hack to Fix

%RiseTimeDiffGrads = str2double(DiffusionInfo.diffusionScheme.diffEncodingRiseTime.Text);
% Calculate b-values
...
%values = bVal(diffGradients,diffBigDelta_us/1E3,diffLittleDelta_us/1E3,RiseTimeDiffGrads/1E3);

bvecs = vertcat(diffDirectionRead, diffDirectionPhase, diffDirectionSlice);
