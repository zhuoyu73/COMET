function [ phase_errors ] = calcMRENavPhaseErrors(rInfo, navigatorImages)
...
%                                shot_navs(:,:,:,ii,jj,kk,ll,mm,nn,oo) = img_nav_HR(:,:,:,ii,jj,kk,ll,mm,nn,oo) - refImage; %AMC
%                            end
%                        end
%                    end
%                end
                    ...
                    for kk = 1:rInfo.nSlices
                        for ii = 1:rInfo.nShots
                            %for hh = 1:rInfo.multibandFactor
                                %shot_navs_temp(:,:,hh,ii,jj,kk,ll,mm,nn) = resample_map_resolution(navigatorImages(:,:,hh,ii,jj,kk,ll,mm,nn,oo),rInfo.N,[],rInfo.FOV*10,rInfo.FOV*10);
                                shot_navs_temp(:,:,:,ii,jj,kk,ll,mm,nn) = resampleNavigators(navigatorImages(:,:,:,ii,jj,kk,ll,mm,nn,oo),rInfo);                   
                            %end
                        end
                    end
                end
                ...
                for mm = 1:rInfo.nPhases
                    for nn = 1:rInfo.nEchoes
                        for oo = 1:rInfo.nRepetitions
                            ...
                            refImage = mean(refImage,4);
                            phase_errors(:,:,:,ii,jj,kk,ll,mm,nn,oo) = angle(shot_navs(:,:,:,ii,jj,kk,ll,mm,nn,oo)./refImage); %AMC
                        end
                    end
                end
            end
