classdef pcSENSE_GPU
    ...
    methods
        function obj = pcSENSE_GPU(A,phaseMaps,sen,varargin)
   ...
   %         end
            ...
            %obj.A.st = []; % Intentionally clear out the spatial interpolator
            % Intialize the field corrected transform objects for shot-wise
            % reconstruction
            for ii = 1:obj.nShots
                % Deal with support mask on the transform
                if obj.A.G.isMasked
                    Gtemp = NUFFT_GPU(obj.A.G.Kx(:,ii),obj.A.G.Ky(:,ii),obj.A.G.Kz(:,ii),obj.A.G.Nx,obj.A.G.Ny,obj.A.G.Nz,'mask',obj.A.G.mask,'UseGPU',obj.UseGPU,'Backend',obj.A.G.Backend,'Precision',obj.Precision);
                else % No support mask used
                    Gtemp = NUFFT_GPU(obj.A.G.Kx(:,ii),obj.A.G.Ky(:,ii),obj.A.G.Kz(:,ii),obj.A.G.Nx,obj.A.G.Ny,obj.A.G.Nz,'UseGPU',obj.UseGPU,'Backend',obj.A.G.Backend,'Precision',obj.Precision);
                end
            ...
            % ---- PATCH START: parse optional GPU/precision flags ----
            if ~isempty(varargin)
            % accept ...,'UseGPU',true,'Precision','single'
                for k=1:2:numel(varargin)
                    key = lower(string(varargin{k}));
                    val = varargin{k+1};
                    if key=="usegpu", obj.UseGPU = logical(val); end
                    if key=="precision", obj.Precision = char(val); end
                end
            end
            % cast sensitivity maps once to target device/precision
            obj.sen = cast(obj.sen, obj.Precision);
            if obj.UseGPU, obj.sen = gpuArray(obj.sen); end
            
            %{
            % also pass flags down into per-shot NUFFT if present
            for ii=1:obj.nShots
                Gii = obj.A_Array{ii}.G;
                if isprop(Gii,'UseGPU'),    Gii.UseGPU = obj.UseGPU; end
                if isprop(Gii,'Backend') && isprop(obj.A,'Backend')
                    Gii.Backend = obj.A.Backend;
                end
                if isprop(Gii,'Precision'), Gii.Precision = obj.Precision; end
            end
            %}
            % ---- PATCH END ----
        ...
        function y = mtimes(obj,x)
            ...
            if ~obj.isTranspose % Forward operation
                ...
                if obj.UseGPU
                    if ~isa(x,'gpuArray')
                        x = gpuArray(cast(x, obj.Precision)); 
                    else
                        x = cast(x, obj.Precision);
                    end
                    yMat = gpuArray.zeros(obj.ShotLength*obj.nCoils, obj.nShots, obj.Precision);
                    j_like = complex(cast(0,obj.Precision), cast(1,obj.Precision));
                else
                    x = cast(x, obj.Precision);
                    yMat = zeros(obj.ShotLength*obj.nCoils, obj.nShots, obj.Precision);
                    j_like = 1j;
                end
                ...
                for indx = 1:obj.nShots
                    ...
                    if obj.A.G.isMasked, P_tmp = P_tmp(obj.A.G.mask); end
                    P_tmp = cast(P_tmp, obj.Precision);
                    if obj.UseGPU
                        P_tmp = gpuArray(P_tmp);
                    end
                    
                    
                    % Build Psense in one shot: [Nimage, nCoils]
                    S = sense(obj.A_Array{indx}, obj.sen .* exp(j_like * P_tmp));
                ...
                if obj.UseGPU
                    ...
                    yShots = gpuArray.zeros(obj.Nimage, obj.nShots, obj.Precision);
                    j_like = complex(cast(0,obj.Precision), cast(1,obj.Precision));
                else
                    x = cast(x, obj.Precision);
                    yShots = zeros(obj.Nimage, obj.nShots, obj.Precision);
                    j_like = 1j;
                end
        ...
        %{
        function checkAdjointness(obj)
        %CHECKADJOINTNESS  验证 SENSE 算子前向和伴随是否满足内积关系
        % <Sx, y> ≈ <x, S'*y>
        
            x = randn(obj.Nimage,1,obj.Precision) + 1j*randn(obj.Nimage,1,obj.Precision);
            y = randn(obj.ShotLength*obj.nCoils*obj.nShots,1,obj.Precision) ...
              + 1j*randn(obj.ShotLength*obj.nCoils*obj.nShots,1,obj.Precision);
            ...
            lhs = sum(conj(Sx(:)) .* y(:));   % <Sx, y>
            rhs = sum(conj(x(:))  .* Sy(:));  % <x, S'*y>
            
            err = abs(lhs - rhs) / abs(lhs);

            fprintf('Adjointness check:\n');
            fprintf('<Sx,y> = %g + %gi\n', real(lhs), imag(lhs));
            fprintf('<x,S''y> = %g + %gi\n', real(rhs), imag(rhs));
