...
set_ADC_len = 1024;  % set length of ADC partitions in sequences (Standard for Sutton Spirals)
...
% Get fname_root and fname for data
    ...
    elseif (~isempty(findstr(fname_list(ii).name,'.dat')))
        ...
        fname_root = fname;
        if exist(fname,'file')
            fname_data = sprintf('%s',fname);
            fname_hdr = sprintf('%s',fname);
        elseif exist(sprintf('%s.dat',fname),'file')
            fname_data = sprintf('%s',fname);
            fname_hdr = sprintf('%s',fname);
        else
           sprintf('Did nof find data file: %s \n',fname)
           return
        end
    else
...
%-------------------------------
	...
	case 1    % 1: constant density spiral
	  ...
	  %nroa = 8*ceil(lgadc/8);
           if (lgadc > 4096)
              num_adc = ceil(lgadc/set_ADC_len);
           elseif (nroa == set_ADC_len)  %((((nro-nroa)/nro)>0.25) & (nroa == set_ADC_len))
              num_adc = ceil(lgadc/set_ADC_len);
           end
