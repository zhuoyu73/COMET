...
%	Am0 = A * Gdiag(mi, 'mask', arg.mask); % jf
	Am0_xn = Am0 * (mi .* EX_xi); %AMC
	GLn = 2*real( 1i*conj(EX_xi) .* (mi.* (Am0'*(yi - Am0_xn))) ); % AMC % dL/dx
	...
	while ((ak==0) || (abs(ak-ak_old)/abs(ak) > arg.thr)) && (k <= arg.subiter)
		...
		Am0_xn_dn = Am0 * (mi.*EX_xi_dn); %AMC
 		tmp1 = 1i*conj(EX_xi_dn) .* (mi.*(Am0'*(yi-Am0_xn_dn))); %AMC
		...
		if arg.btr == 1 % backtracking
			...
			while 1
				...
				fa1 = norm(yi-Am0*(mi.*EX_xi_dn1))^2 + beta*R1x;
				if (fa1-fa0) < 1e-8
					break
				end
				fa0 = fa1;
