...
if ~isvar('mt')
    ...
    beta22 = 0.5*2^-5 * curv; % regularization paramter for magnitude
	if edgepr == 1;
...
if ~isvar('A')
    ...
    [kx,ky] = meshgrid(-Nd(1)/2:Nd(1)/2-1,-Nd(2)/2:Nd(2)/2-1);
    kz = ones(size(kx));
    A = NUFFT(kx(samp(:)), ky(samp(:)), kz(samp(:)),Nd(1),Nd(2),1,'mask',mask);

	%% Setup the wavlet transform object
	% U = Gwave1('mask', mask, 'noCA', 0, 'level', 3);
	%U = Godwt1(mask, 'level', 3); % jf
    U = Wavelet2(Nd(1),Nd(2),mask,5);
