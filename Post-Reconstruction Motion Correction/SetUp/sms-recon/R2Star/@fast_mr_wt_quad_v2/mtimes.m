...
if ~a.is.transpose
    vi = a.HH*double(vi(:));   % oversampled in either direction
    vo = zeros(size(tt(:)));
    
    %wec = squeeze(we(:,:,ii));
    %r2c = squeeze(r2(:,:,ii));
      
    vic = exp(-(i*we +r2)*TE).*reshape(vi(:),N1,N2,N3);
    
    for ll = 1:L
        Wo = exp(-(i*we+r2).*((ll-1)*tau));
        aa = Int(ll,:).';
        vo = vo + aa.*(G*(Wo.*vic));
    end
    ...
    vo = zeros(size(we,1),size(we,2),size(we,3));
    
    %wec = squeeze(we(:,:,ii));
    %r2c = squeeze(r2(:,:,ii));
    
    for ll = 1:L
        Wo = exp((i*conj(we)-r2).*((ll-1)*tau));
        aa = Int(ll,:)';
        
        if ll == 1
            voc = Wo.*reshape(G'*(aa.*vi(:)),N1,N2,N3);
        else
            voc = voc + Wo.*reshape(G'*(aa.*vi(:)),N1,N2,N3);
        end
        
    end
    vo = vo +exp((i*conj(we)-r2).*TE).*voc;
