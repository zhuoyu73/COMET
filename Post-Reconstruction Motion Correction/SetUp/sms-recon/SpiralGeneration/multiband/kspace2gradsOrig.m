function [gx gy gz] = kspace2gradsOrig(kx,ky,kz,Nx,Ny,Nz, tsamp, lRampUpPts, lRampDownPts)
...
for ll=1:lshots
    if kx(1,ll) == 0;
        ...
        if (max(kx(:,ll))==min(kx(:,ll))) %readout is only a phase encode
            Gpeak = kx(1,ll)/FOVref/gambar/(lRampUpPts-1)/(gts*1e-6)*2;
            ...
            gx_pre(lRampUpPts,ll) = diff(kx(1:2,ll))/gambar/FOVref/tsamp*1e6;
            ...
            Garea = kx(1,ll)/FOVref/gambar/(gts*1e-6)-sum(gx_pre((lRampUpPts-pts_for_ramp):end,ll))-gx_pre(lRampUpPts,ll)*(tsamp/gts);
    ...
    kx_pre(1,ll) = sum([0; gx_pre(:,ll)])*gts*FOVref*gambar*1e-6;
    
    if ky(1,ll) == 0;
        gy_pre(:,ll) = 0;
    else
        if (max(ky(:,ll))==min(ky(:,ll))) %readout is only a phase encode
            Gpeak = ky(1,ll)/FOVref/gambar/(lRampUpPts-1)/(gts*1e-6)*2;
            if mod(lRampUpPts,2)== 0
                Gpeak = Gpeak*(1+1/(lRampUpPts*(lRampUpPts-2)));
            end
            for xx = 1:(lRampUpPts)
            ...
            gy_pre(lRampUpPts,ll) = diff(ky(1:2,ll))/gambar/FOVref/tsamp*1e6;
            pts_phase_blip = floor(lRampUpPts*2/3);
            pts_for_ramp = floor(lRampUpPts/3);
            for xx = 1:pts_for_ramp
                gy_pre(lRampUpPts+1-xx,ll) = gy_pre(lRampUpPts,ll)/(pts_for_ramp-1)*(pts_for_ramp-xx);
            end
            Garea = ky(1,ll)/FOVref/gambar/(gts*1e-6)-sum(gy_pre((lRampUpPts-pts_for_ramp):end,ll))-gy_pre(lRampUpPts,ll)*(tsamp/gts);
            Gpeak = Garea/(pts_phase_blip-1)*2;
            if mod(pts_phase_blip,2)== 0
                Gpeak = Gpeak*(1+1/(pts_phase_blip*(pts_phase_blip-2)));
            end
    ...
    ky_pre(1,ll) = sum([0; gy_pre(:,ll)])*gts*FOVref*gambar*1e-6;
   ...
   if kz(1,ll) == 0;
        ...
        if (max(kz(:,ll))==min(kz(:,ll))) %readout is only a phase encode
            Gpeak = kz(1,ll)/FOVref/gambar/(lRampUpPts-1)/(gts*1e-6)*2;
            if mod(lRampUpPts,2)== 0
                Gpeak = Gpeak*(1+1/(lRampUpPts*(lRampUpPts-2)));
            end
            for xx = 1:(lRampUpPts)
            ...
            gz_pre(lRampUpPts,ll) = diff(kz(1:2,ll))/gambar/FOVref/tsamp*1e6;
            pts_phase_blip = floor(lRampUpPts*2/3);
            pts_for_ramp = floor(lRampUpPts/3);
            for xx = 1:pts_for_ramp
                gz_pre(lRampUpPts+1-xx,ll) = gz_pre(lRampUpPts,ll)/(pts_for_ramp-1)*(pts_for_ramp-xx);
            end
            Garea = kz(1,ll)/FOVref/gambar/(gts*1e-6)-sum(gz_pre((lRampUpPts-pts_for_ramp):end,ll))-gz_pre(lRampUpPts,ll)*(tsamp/gts);
            Gpeak = Garea/(pts_phase_blip-1)*2;
            if mod(pts_phase_blip,2)== 0
                Gpeak = Gpeak*(1+1/(pts_phase_blip*(pts_phase_blip-2)));
            end
    ...
    kz_pre(1,ll) = sum([0; gz_pre(:,ll)])*gts*FOVref*gambar*1e-6;
end

%convert kspace to gradients;
gx = diff([kx_pre(:,:); kx])/gambar/FOVref/tsamp*1e6;
gy = diff([ky_pre(:,:); ky])/gambar/FOVref/tsamp*1e6;
gz = diff([kz_pre(:,:); kz])/gambar/FOVref/tsamp*1e6;
...
for ll = 1:lshots
    ...
    kx(:,ll)=kxt*1e-6+kx_pre(:,ll);
    ky(:,ll)=kyt*1e-6+ky_pre(:,ll);
    kz(:,ll)=kzt*1e-6+kz_pre(:,ll);
end
...
FOVminX = floor(FOVref/maxGrad*Gref);
FOVminY = floor(FOVref/maxGrad*Gref);
FOVminZ = floor(FOVref/maxGrad*Gref);

%check to make sure slew rate is not exceeded TODO

gInfo = [Gref; FOVref; ladc; tsamp; FOVminX; FOVminY; FOVminZ; lshots; Nx; Ny; Nz; tCenter; lgradpts; lRampUpPts; lRampDownPts];
