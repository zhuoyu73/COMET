function mrprot = parse_xprot1(rawarr)
...
%if ~isempty(xstart)            
%    workarr = extractBraceString(rawarr(xstart(1):end));
...
while (~isempty(workarr))
    ...
    if (strncmpi(tagType, 'ParamMap', 8) || strncmpi(tagType, 'Pipe', 4) || strncmpi(tagType, 'PipeService',11) || strncmpi(tagType, 'ParamFunctor',12))
        if (~isempty(tagStr))
            level = level + 1;
...
%         stubArr = stubArr(length(remArr)+1:end);
    ...
    elseif (strncmpi(tagStr, 'EVAStringTable', 14))
        ...
        stubStr = extractBraceString(workarr);
        workarr = workarr(length(stubStr)+1:end);
    elseif strncmpi(tagStr, 'Class', 5)
        % Do nothing!
...
function stvar = extractBraceString(text)
    ...
    %    %---- 把原先的两行改成只取第一个索引 ----
    %    tstart = find(text == '{', 1, 'first');
    %if isempty(tstart)
    %    stvar = '';              % 没有左括号，直接返回空
    %    return
    %end
    
    %tlen = length(text);         % 继续扫到整段文本末尾
    
    tlen = length(text); %original
    tstart = strfind(text,'{'); %original
...
%ZS 0804
%startPos = find(inStr=='<', 1, 'first'); 
%if isempty(startPos), return, end
%endPos = find(inStr(startPos+1:end)=='>', 1, 'first');
%if isempty(endPos), return, end
