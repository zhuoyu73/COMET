                    ...
                    for nn = 1:rInfo.nEchoes
                        for oo = 1:rInfo.nRepetitions
                            for qq = 1:rInfo.nCoils
                                ...
                                navData(:,ii,jj,kk,ll,mm,nn,oo,qq) = curData.*exp(-1j*((rInfo.phaseShift(rInfo.ExcOrder(kk))+rInfo.halfVoxelShiftNavPhase).*rInfo.kPhaseNav(:,ii,jj,kk,ll,mm,nn,oo)+(rInfo.readShift(rInfo.ExcOrder(kk))+rInfo.halfVoxelShiftNavRead).*rInfo.kReadNav(:,ii,jj,kk,ll,mm,nn,oo)+(rInfo.sliceShift(rInfo.ExcOrder(kk))+rInfo.halfVoxelShiftNavSlice).*rInfo.kSliceNav(:,ii,jj,kk,ll,mm,nn,oo))*2*pi);
                            end
                        end
                    end
                end
        ...
        for mm = 1:length(phasesToRecon)
            ...
            for nn = 1:length(echoesToRecon)
                ...
                for oo = 1:length(repetitionsToRecon)
                    ...
                    rInfo.dataMaskNav = rInfo.dataMaskNav(1:3986);
                    ...
                    for ii = 1:rInfo.nShots
                        for jj = 1:rInfo.nPartitions
                            ...
                            if rInfo.nPartitionsNav > 1
                                G = NUFFT(col(rInfo.kReadNav(rInfo.dataMaskNav,ii,jj,slc,avg,phs,eco,rep)), ...
                                    col(rInfo.kPhaseNav(rInfo.dataMaskNav,ii,jj,slc,avg,phs,eco,rep)), ...
                                    col(rInfo.kSliceNav(rInfo.dataMaskNav,ii,jj,slc,avg,phs,eco,rep)), ...
                                    rInfo.NNav, ...
                                    rInfo.NNav, ...
                            ...
                            %                             end
%                             data = squeeze(data);
%                             for coil = 1:rInfo.nCoils
%                                 cImages(:,ii) = G'*(dcf(length(dcf)-length(data)+1:end).*col(data(:,ii)));
%                             end
                            ...
                            %nav = mi.*exp(1j*phi);
                            img(:,:,:,ii,jj,kk,ll,mm,nn,oo) = embed(nav,logical(mask(:,:,:,kk)));
                            toc
                        end
                    end
                end
