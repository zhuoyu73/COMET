function [img] = fieldCorrectedNavReconCIVIC_PGSE(rInfo, sen, mask, FM, varargin)
                    ...
                    for nn = 1:rInfo.nEchoes
                        for oo = 1:rInfo.nRepetitions
                            for qq = 1:rInfo.nCoils
                                ...
                                navData(:,ii,jj,kk,ll,mm,nn,oo,qq) = curData.*exp(-1j*(rInfo.readShift(rInfo.ExcOrder(kk)).*rInfo.kReadNav(:,ii,jj,kk,ll,mm,nn,oo)+rInfo.phaseShift(rInfo.ExcOrder(kk)).*rInfo.kPhaseNav(:,ii,jj,kk,ll,mm,nn,oo)-(rInfo.sliceShift(rInfo.ExcOrder(kk))-halfVoxelShift).*rInfo.kSliceNav(:,ii,jj,kk,ll,mm,nn,oo))*2*pi);
                            end
                        end
                    end
                end
            ...
            for nn = 1:length(echoesToRecon)
                ...
                for oo = 1:length(repetitionsToRecon)
                    ...
                    if rInfo.nPartitionsNav > 1
                        ...
                        %                              A = Gdft_r2(col(rInfo.kReadNav(rInfo.dataMaskNav,ii,jj,slc,avg,phs,eco,rep)), ...
                        %                                      col(rInfo.kPhaseNav(rInfo.dataMaskNav,ii,jj,slc,avg,phs,eco,rep)), ...
                        %                                      col(rInfo.kSliceNav(rInfo.dataMaskNav,ii,jj,slc,avg,phs,eco,rep)), ...
                        %                                      rInfo.NNav, ...
                        %                                      rInfo.NNav, ...
