...
%	Am0 = A * diag_sp(mi);
    ...
    GLn = 2*real( 1i*conj(EX_xi) .* conj(mi) .*(A'*(yi - Am0_xn)) );
	Cx = double(C * xi);
	w1 = double(wei_func_gf(Cx, del));
%	GRn = beta * diag_sp(w1) * Cx;  % gradient for beta*R(x(n))
	GRn = double(beta * (w1 .* Cx)); % jf

	Gn = double(GLn + C' * GRn); % gradient for Psi(x(n))
	...
	while ((ak==0) || (abs(ak-ak_old)/abs(ak) > arg.thr)) && (k <= arg.subiter)
		...
		tmp = 1i*conj(EX_xi_dn) .* conj(mi) .* (A'*(yi-Am0_xn_dn));
		if ak ~= 0
			GLn = 2*real( tmp );
			C_xi_dn = double(C*xi + ak*pn);
